...
export const useAllAccounts = () => {
  ...
  const fetchAccounts = async () => {
    try {
      const res = await api.get<AccountSummary[]>("/admin/account");
...
export const createAccount = async (
  ...
  try {
    const res = await api.post<AccountSummary>("/admin/account", {
      name,
      password,
    });
    return res.data;
...
export const changeAccountPassword = async (
  ...
  try {
    const res = await api.put<{ message: string }>("/admin/password", {
      id,
      password,
    });
    return res.data;
